    ...
    validationSchema: Yup.object({
      ...
      username: Yup.string()
        ...
        .test("SpaceInName", "Invalid Username", function (value) {
          if (value != null) return !value.includes(" ");
        })
        .test("UserFound", "Username is Taken", async function (value) {
          if (value != null) return !(await userCheck(value));
        }),
      password: Yup.string()
        .required("Required Field")
        ...
        .test(
          "InvalidCriteria",
          "Password must contain a Number, Special, Uppercase, and Lowercase Character",
          function (value) {
      ...
      <div className={styles.container}>
        <form className={styles.signup_form} onSubmit={formik.handleSubmit}>
          ...
          {formik.errors.first && formik.touched.first ? (
            <span className={styles.error}>{formik.errors.first}</span>
          ) : (
            <></>
          )}
          ...
          {formik.errors.last && formik.touched.last ? (
            <span className={styles.error}>{formik.errors.last}</span>
          ) : (
            <></>
          )}
          ...
          {formik.errors.confirm_email && formik.touched.confirm_email ? (
            <span className={styles.error}>{formik.errors.confirm_email}</span>
          ) : (
            <></>
          )}
